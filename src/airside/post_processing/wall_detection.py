...
def find_walls(filename):
    ...
    print(f"Found {len(extracted_planes)}.")
    print("Merging coplanar patches...")
    ...
    for patch in extracted_planes:
        ...
        for wall_group in merged_walls:
            for existing_model in wall_group['models']:
                if is_coplanar(patch['model'], existing_model, angle_tol_deg=current_angle_tol, dist_tol=current_dist_tol):
    ...
    print(f"Merged into {len(merged_walls)} distinct physical walls.")

    # Sort walls by size so the largest walls
    merged_walls.sort(key=lambda group: sum([len(cloud.points) for cloud in group['clouds']]), reverse=True)
    ...
    for i in range(len(merged_walls)):
        ...
        for c in merged_walls[i]['clouds']: cloud_i += c
        ...
        for j in range(i + 1, len(merged_walls)):
            ...
            for c in merged_walls[j]['clouds']: cloud_j += c
            ...
            if 60 < acute_angle < 120:
                ...
                if min_dist < corner_proximity_threshold:
                    print(f" -> Corner detected: Wall {i+1} & Wall {j+1} (Gap: {min_dist:.2f} units, Initial Angle: {acute_angle:.1f} deg)")
                    
                    if j not in adjusted_indices:
                        print(f"    Squaring Wall {j+1} to exactly 90 degrees relative to Wall {i+1}...")
                        ...
                        merged_walls[j]['models'][0] = [n_j_ortho[0], n_j_ortho[1], n_j_ortho[2], d_ortho]
                        ...
                        for cloud in merged_walls[j]['clouds']:
                            ...
                            micro_noise = np.random.normal(0, 1e-5, projected_points.shape)
                            cloud.points = o3d.utility.Vector3dVector(projected_points + micro_noise)
    ...
    for i, wall_group in enumerate(merged_walls):
        ...
        combined_cloud.paint_uniform_color([r, g, b])
        visual_elements.append(combined_cloud)
