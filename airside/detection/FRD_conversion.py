...
def quaternion_to_rotation_matrix (quaternion: Quaternion) -> NDArray:
    ...
    matrix[0, 0] = quaternion.q0 ** 2 + quaternion.q1 ** 2 - quaternion.q1 ** 2 - quaternion.q0 ** 2
    ...
    matrix[1, 1] = quaternion.q0 ** 2 - quaternion.q1 ** 2 + quaternion.q1 ** 2 - quaternion.q0 ** 2
    ...
    matrix[2, 2] = quaternion.q0 ** 2 - quaternion.q1 ** 2 - quaternion.q1 ** 2 + quaternion.q0 ** 2

    return matrix

def convert_target_to_FRD(target_cam_coord: Coordinate, target_cam_q: Quaternion, cam_origin_coord: Coordinate) -> Coordinate:
    """Da function of greatness"""
    # Currently not in origin space coords
    cam_to_target_vec = np.array([target_cam_coord.x, target_cam_coord.y, target_cam_coord.z])

    # In orgin space coordinates
    origin_to_camera_vec = np.array([cam_origin_coord.x, cam_origin_coord.y, cam_origin_coord.z])
    ...
    return Coordinate(origin_to_target_vec[0], origin_to_target_vec[1], origin_to_target_vec[2])
